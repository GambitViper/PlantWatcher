...
namespace RaspberryPiHeadless
{
    public sealed class StartupTask : IBackgroundTask
    {
        ...
        private const string ServiceAddress = "http://plantwatcherbot.azurewebsites.net/api/";
        ...
        private DateTime heatLampTurnOffTime;
        private DateTime boilerTurnOffTime;
        ...
        private async Task SaveSensorReadingToCloud(SensorReading curReading)
        {
        ...
        private void CheckConditionsForSwitches(SensorReading reading)
        {
        ...
        private bool ShouldHeatLampBeOn(double temperatureF)
        {
            if (IsDayTime())
            {
                if (HeatLamp_Value == GpioPinValue.High)
                {
                    return !IsWarmEnough(temperatureF);
                }
                else
                {
                    return IsTooCold(temperatureF);
                }
            }
            else
            {
        ...
        private bool ShouldHumidifierBeOn(bool isOn, double humidity)
        {
            if (isOn)
            {
                return !IsHumidEnough(humidity);
            }
            else
            {
                return IsDry(humidity);
            }
        }

        private bool IsWarmEnough(double tempF)
        {
            return tempF > 77;
        }

        private bool IsTooCold(double tempF)
        {
            return tempF < 73;
        }

        private bool IsDry(double humidity)
        {
            return humidity < 70;
        }

        private bool IsHumidEnough(double humidity)
        {
            return humidity > 80;
        }
